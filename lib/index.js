  ...
  start: function() {
  	...
  	//Get Initial Players
		...
		this.bot.onFollowed(function(event) {
			if(!players.find(function(element) {
		...
		this.bot.onNamedTweet(null, function(event) {
			...
			if(containsHashtag(hashtags, "killed")) {
				var killed = getPlayer(mentions[0].screen_name);
				var killer = getPlayer(user.screen_name);

				if(killed && killer)
					killed.kill(killer);
			}
			else if(containsHashtag(hashtags, "killedby")) {
				var killer = getPlayer(mentions[0].screen_name);
				var killed = getPlayer(user.screen_name);
